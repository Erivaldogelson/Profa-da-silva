...
async function buildOAuthUser(profile, provider, emailFallback = "") {
  const email =
  ...
  passport.use(
    new GoogleStrategy(
      ...
      async (accessToken, refreshToken, profile, done) => {
        try {
          const user = await buildOAuthUser(profile, "google");
          done(null, user);
        } catch (error) {
          done(error);
        }
    ...
    new AppleStrategy(
      ...
      async (accessToken, refreshToken, idToken, profile, done) => {
        try {
          ...
          const user = await buildOAuthUser(profile, "apple", email);
          done(null, user);
        } catch (error) {
          done(error);
        }
...
initStore()
  ...
  .catch((error) => {
    console.error("Erro ao iniciar banco SQL:", error);
    process.exit(1);
  });
